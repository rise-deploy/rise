...
| `RISE_CONTAINER_HOST__<NAME>` | In-cluster address (`<host>:<port>`) of each routable container in a [multi-container deployment](../deployments#multi-container-deployments) — one entry per container that exposes `http_port`. `<NAME>` is the uppercase container name (dashes mapped to underscores). Workers (containers without `http_port`) are excluded. Each container also sees its own entry. | `api:8080` |
...
`RISE_CONTAINER_HOST__<NAME>` values are `host:port` (no scheme), so callers prepend their own protocol — e.g. `redis://${RISE_CONTAINER_HOST__REDIS}` or `http://${RISE_CONTAINER_HOST__API}`.
