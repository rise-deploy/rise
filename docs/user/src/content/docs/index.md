---
title: Rise User Docs
description: Learn how to deploy and operate applications on Rise.
---

Rise is a Kubernetes-based platform for deploying containerized applications.

Use these docs when you are building, configuring, deploying, or troubleshooting applications on Rise.

## Start with

- [Getting Started](user-guide/getting-started) to deploy your first application.
- [Project Configuration](user-guide/configuration) to configure `rise.toml`.
- [Deployments](user-guide/deployments) for the deployment lifecycle.
- [CLI Reference](user-guide/cli-reference) for command details.
