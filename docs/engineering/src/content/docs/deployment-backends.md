...
| HTTP health checks | `health_check` | ✅ | ✅ | Readiness/liveness probes (K8s) vs. Traefik per-server health check + controller HTTP probe (Docker). In the default `health-rolling` cutover mode the Docker backend offloads checking to Traefik, honoring `path`, `period_seconds` (→ Traefik `interval`) and `timeout_seconds` (→ Traefik `timeout`); in `recreate` mode Rise's own probe runs once per reconcile tick (honors `path` and `disabled`) and `period_seconds`/`timeout_seconds` are ignored. The remaining knobs — `initial_delay_seconds`, `failure_threshold` and the separate `liveness_enabled`/`readiness_enabled` toggles — are K8s-only. |
...
| Workload identity tokens | token-exchange API + `[identity].audiences` | ✅ | ✅ | Both backends deliver the bootstrap credential + per-audience token files to `/var/run/secrets/rise/identity/` and refresh the tokens before they expire. K8s mounts a per-deployment Secret as a volume; Docker writes the same files via the Docker archive API (`PUT /containers/{id}/archive`) right after create, recovering the credential from the running container across recreates. The token-exchange endpoint is backend-agnostic. |
