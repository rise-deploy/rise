...
- **[Amazon ECS](/operator-docs/ecs/)** — the AWS counterpart. Runs apps as ECS
  tasks routed by Traefik's ECS provider, on Fargate (nothing to operate) or on
  container instances you already run. Designed in
...
development; ECS when you are on AWS — on Fargate to avoid running a cluster at
all, or on EC2 capacity to deploy into one you already have.
...
| CPU / memory limits | `cpu`, `memory` | ✅ | ✅ | ⚠️ | Pod resources (K8s) vs. `nano_cpus`/`memory` (Docker) vs. task size (ECS). **On `capacity: fargate`, ECS accepts only a fixed table of CPU/memory pairs**, so a request is rounded **up** to the smallest valid combination — never down. This is billing-visible: Rise's defaults (`500m`/`256Mi`) resolve to 0.5 vCPU / 1 GB, because 512 CPU units require ≥1024 MiB. The resolved size is logged. A request beyond the largest Fargate size fails the deployment with that reason on its first reconcile. **On `capacity: ec2` there is no table** — the request is declared exactly as written, and the bound is what the container instance has, so an unplaceable task surfaces as an ECS placement event on the timeline rather than a deploy-time rejection. The `⚠️` is the Fargate rounding; EC2 capacity is `✅`. |
...
| Deployment observability (Timeline + Containers tabs) | `GET .../deployments/{id}/events`, `GET .../deployments/{id}/containers` | ✅ | ✅ | ⚠️ | Two views over one model. **Containers** is a snapshot — every backend reports each replica's state, start/finish, health and image, keyed by its own stable handle: `web[0]` on Docker (a replica index it keeps in a label), the pod name on Kubernetes, the task id on ECS. **Timeline** is the history derived from consecutive snapshots, plus the runtime's own events. **ECS reports no restart counter** — ECS replaces a task rather than restarting a container, on either capacity, so a restart appears as one task ending and another starting rather than a counter advancing, and no replica ordinal is exposed because ECS has none. Docker forwards no runtime events yet (its daemon stream is a persistent connection rather than a poll); Kubernetes and ECS forward theirs. |
...
| Workload identity tokens | token-exchange API + `[identity].audiences` | ✅ | ✅ | ✅ | All three deliver the bootstrap credential + per-audience token files to `/var/run/secrets/rise/identity/` — the credential to every deployment, whether or not it declares audiences — and refresh the tokens before they expire. K8s mounts a per-deployment Secret as a volume and a leader-elected controller loop resyncs a project when one of its deployments is due (~2/3 of `identity_token_ttl_seconds` after each mint) so the sync webhook re-mints the token before it expires — Metacontroller does not resync a steady project on its own; Docker writes the same files via the Docker archive API (`PUT /containers/{id}/archive`) right after create and re-mints on its own reconcile loop, recovering the credential from the running container across recreates. ECS has no way to write into a running task, so every task runs an **identity sidecar** (`rise identity agent`, the Rise image) that writes the files onto a task volume the app mounts read-only; the app starts only once they exist. The control plane mints the credential into SSM before the service exists, and the sidecar mints one token per declared audience from the backend-agnostic token-exchange endpoint (`POST /api/v1/identity/token`), whose cap is the same `identity_token_ttl_seconds` the other two use for the files — so lifetime and claims match. Refresh runs in the sidecar rather than the controller. **ECS caveat:** a deployment already running before the sidecar shipped keeps running without one (adding it would roll every service at once) and gets it on its next deploy. |
...
| Per-container log filter (`--container`, `?container=`, container chips) | `deployment_logs` | ✅ | ✅ | ✅ | Every line carries the container that produced it, and the filter narrows to one or more of the deployment's containers (`app` for a single-container deployment). ECS resolves both from the awslogs stream name (`{stream_prefix}/{container}/{task-id}`) and pushes a one-container filter down as the stream prefix; Loki resolves both from the `labels.container` stream label — a shipper that doesn't emit it serves unattributed lines and matches nothing when filtered. Kubernetes and Docker have no multi-workload log endpoint to call — each container of a deployment is its own Pod/container (on Kubernetes, so is each replica) — so Rise opens one reader per workload and merges them, the way `docker compose logs` and `kubectl logs --all-containers` do. A finite range is sorted by timestamp before it is returned; **a followed stream is emitted in arrival order**, because ordering a live merge means holding lines back, and the container named on each line is what puts them back together. |
