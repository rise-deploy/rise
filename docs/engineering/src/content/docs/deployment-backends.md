...
| HTTP health checks | `health_check` | ✅ | ✅ | Readiness/liveness probes (K8s) vs. Traefik per-server health check + controller HTTP probe (Docker). The Docker backend offloads checking to Traefik, honoring `path`, `period_seconds` (→ Traefik `interval`) and `timeout_seconds` (→ Traefik `timeout`); when no Traefik API is reachable Rise's own probe mirrors it (honors `path` and `disabled`). The remaining knobs — `initial_delay_seconds`, `failure_threshold` and the separate `liveness_enabled`/`readiness_enabled` toggles — are K8s-only. |
