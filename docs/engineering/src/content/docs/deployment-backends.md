...
| HTTP health checks | `health_check` | ✅ | ✅ | Readiness/liveness probes (K8s) vs. Traefik per-server health check + controller HTTP probe (Docker). In the default `health-rolling` cutover mode the Docker backend offloads checking to Traefik, honoring `path`, `interval` and `timeout`; in `recreate` mode Rise's own probe runs once per reconcile tick (honors `path` and `disabled`). The fine-grained threshold knobs (`period_seconds`, `failure_threshold`, …) and separate `liveness_enabled`/`readiness_enabled` toggles are K8s-only. |
