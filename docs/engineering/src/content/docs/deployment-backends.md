...
| Runtime logs (`rise deploy logs`, Logs tab) | `deployment_logs` | ✅ | ✅ | ⚠️ | Every runtime-native variant needs a client the backend has: a kube client (`kubernetes`) or the shared bollard handle (`docker`). **ECS has neither**, so [`config/ecs.yaml`](https://github.com/rise-deploy/rise/blob/develop/config/ecs.yaml) ships `deployment_logs.type: none` — the API answers `historical_backend_not_configured` and the UI renders its empty state, rather than the install crashing at startup on the `kubernetes` default. App containers still log to CloudWatch via the `awslogs` driver (ADR-0005 D11); nothing reads them back yet. ECS installs that want logs today can set `type: loki`, which is backend-agnostic (it queries by stream label, not by runtime client). A CloudWatch variant is the intended replacement — a tracked gap, not a fundamental limitation. |
