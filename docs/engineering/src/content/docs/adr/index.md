...
| [ADR-0004](./0004-control-plane-process-topology.md) — Control-Plane Process Topology | Proposed | The resource API, admission, authorization, GC, and watch become a `rise-apiserver` process; everything else is a client over the HTTP contract with a Rise-issued token. Fixes the invariants, the gates, and the order the split happens in. |
| [ADR-0005](./0005-ecs-deployment-backend.md) — ECS Deployment Backend | In Progress | How Rise's deployment model maps onto Amazon ECS (Fargate) and the surrounding AWS services: Traefik ingress behind an AWS load balancer, ECR pulls via the task execution role, SSM-injected secret env, and a sidecar-delivered workload identity contract. |
| [ADR-0006](./0006-source-available-backend-licensing.md) — Source-Available Backend Licensing | Implemented | The core, CLI and Docker backend stay MIT OR Apache-2.0; the Kubernetes and ECS backends become BUSL-1.1 converting to the same dual license after four years. Draws the boundary at crate edges, which obliges extracting the in-tree Kubernetes backend. |
