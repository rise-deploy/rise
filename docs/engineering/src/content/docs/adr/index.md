...
| [ADR-0004](./0004-ecs-deployment-backend.md) — ECS Deployment Backend | In Progress | How Rise's deployment model maps onto Amazon ECS (Fargate) and the surrounding AWS services: Traefik ingress behind an AWS load balancer, ECR pulls via the task execution role, SSM-injected secret env, and a sidecar-delivered workload identity contract. |
