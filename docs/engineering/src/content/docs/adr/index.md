...
| [ADR-0006](./0006-deployment-event-log.md) — Deployment Event Log | Draft | An append-only log of deployment events beside the status column and the observed-state snapshot, so history survives the tick that observed it. Status changes are written where status is written; replica events are the diff against the previous snapshot. |
