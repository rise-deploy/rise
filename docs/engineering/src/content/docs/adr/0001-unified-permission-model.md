...
Max token TTL is governed by the same four-layer stack (§5), composed via `min()`: an SA-specific setting, bounded by the org's own declared TTL ceiling, bounded by the operator's per-org imposed ceiling, bounded by the instance-wide default. The instance-level maximum (Layer 1) is mandatory and finite — never unset — so the `min()` composition is always over a non-empty set and no misconfiguration can yield an unbounded-TTL token. Setting an SA-specific TTL that exceeds the resolved ceiling is **rejected at write time**, consistent with how Layer 4 treats permission-set grants (§5) — it is never silently accepted and clamped later; a write that would be meaningless once capped is refused up front rather than stored as a misleading value. As with permission-set grants, the writer must also currently hold an equal-or-greater TTL entitlement themselves — the numeric case of the same writer-subset check.
...
references:
  - at:        spec.runtimeClass     # a field path or a label key
    kind:      RuntimeClass
    verb:      use
    liveCheck: false                 # snapshot (default) vs. re-checked on reconcile
```
...
By default this deliberately gives the reference *snapshot* semantics, not §6.1's live semantics: the never-store rule exists for access-driving labels, where staleness is a security bug, whereas here the recorded value is the *output* of a decision made at a specific moment by a specific subject, and reproducibility is the point. The org's default label remains live as an *input* to the next deployment. Revoking an org's `use` grant therefore stops the *next* deployment, never a running one — consistent with Layer 4 grants being write-time everywhere else (§5), and the right availability call: a revoked class ages out at the org's next deploy or rollback (which creates a new deployment and re-resolves against current grants).

**Snapshot vs live is a per-reference property.** Whether a materialized reference is a point-in-time *snapshot* or re-checked *live* is declared per reference (`liveCheck: true|false`), defaulting to snapshot (`false`) — correct for availability-only referents like `RuntimeClass`, whose selection is not itself a security boundary. For a reference marked `liveCheck: true`, the consumer's `use` is re-evaluated on every reconcile (or bounded by a maximum materialization lifetime), so a revoked `use` grant is revoked within a bounded window rather than lingering indefinitely — the reference analogue of §7's deliberately short token TTLs, and a weaker guarantee than §6.1's per-request liveness, but bounded. Snapshot semantics are safe *only* where the selection is not a security boundary; a security-sensitive referent must set `liveCheck: true`.
...
- The operator tier cannot be locked out of an org: `system:operators` cannot
  be named by an ordinary binding, and its effective policy is the evaluator's
  unconditional `{Allow: * on *}`, ignoring any `Deny` that would otherwise
  catch it (§1).
...
- The transparency the model is designed around (§1's explain/audit tooling —
  ...
  model assumes.
- `use` revocation takes effect at the next deployment for a snapshot
  (`liveCheck: false`) reference — references are materialized and checked at
  write time, so running workloads are never retroactively broken, which also
  means a revoked class lingers until the org's next deploy or rollback. Only a
  `liveCheck: true` reference bounds revocation to a re-check window; snapshot
  semantics are safe only where selection is not a security boundary (§9).
...
40. **No binding, no ownership — two distinct cases.**
    - **(40a) No selecting binding, no gate.** Given a resource where *no* binding's `LabelSelector` references `rise.dev/owner` at that location in the tree, then the label confers no access whatsoever, §6.6 step 2 is skipped, and writing the label requires only ordinary `update` — labels carry no authorization semantics of their own; "ownership" is entirely the effect of a selecting binding (§6, §6.6).
    - **(40b) Selecting binding present but granting nothing.** Given a §6.5 override binding on `{key: rise.dev/owner}` whose Role grants nothing (so a selecting binding *is* present — the override by construction carries `LabelSelector: {key: rise.dev/owner}`), then §6.6 step 2 *does* trigger, but step 3's before/after diff over that binding's grant is empty, so the write passes with no additional permission needed (§6, §6.6).
...
42. **Org binding cannot Deny/DoS a foreign or org-agnostic subject.** Given an org-parented binding under `acme-corp` whose `Subject` is a shared `Controller` (or any `system:`-reserved authority name), when written, then it is rejected — a `Controller` is a member of no single org and so cannot be targeted by an org binding, and `system:`-authority names are reserved; an org cannot Deny or otherwise degrade a cross-org subject through its own bindings (§1).
...
47. **Deny survives replacement.** Given a platform wildcard binding for subject `S` carrying a `Deny`, and an org-specific binding for the same `S` and `LabelSelector` key that restates only Allow content, when evaluating in that org, then the org binding's Allow replaces the wildcard's Allow but the platform `Deny` is preserved — replacement never strips a superseded binding's `Deny`, and cross-placement bindings do not replace each other at all (§1).
...
54. **`liveCheck` reference re-checks; snapshot does not.** Given a reference declared `liveCheck: true`, when the consumer's `use` grant is revoked, then the materialization fails at the next reconcile (bounded revocation); given a reference declared `liveCheck: false`, the resolved value is snapshotted at write time and a later `use` revocation does not disturb the running resource (§9).
