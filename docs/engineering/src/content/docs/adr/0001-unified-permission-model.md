...
**Subject records and their relationships are built-in resources.** All persisted identity, membership, and authentication-policy objects use the existing built-in `rise.dev/v1alpha1` API group; there is no separate `authorization.rise.dev` group. The entire `rise.dev` API group is reserved for platform-owned built-ins, including future kinds; extension `ResourceDefinition`s must use another group. Placement is fixed by kind:
...
| `GroupMembership` | `Group` | One name-bound membership marker for a `User` |
...
The separate trust-policy kinds are intentional: a generic kind has exactly one declared parent, so one `TrustPolicy` kind cannot be parented under both Controller and ServiceAccount without violating the store's exact-parent invariant. Likewise, Group membership is one child resource per edge rather than an array in `Group.spec`: this avoids whole-object lost updates, gives each membership its own grant-gated lifecycle/audit identity, and supports lookups in both directions. A `GroupMembership` is an empty marker whose immutable `metadata.name` must equal the canonical name of an existing User when the membership is created. The ordinary per-parent resource-name uniqueness rule therefore permits at most one edge for a User in a Group. Changing the member is a delete plus create, so adding the replacement User passes the ordinary membership grant gate. Membership itself is boolean for authorization — any descriptive membership role is ordinary metadata and never a second permission system.

**Owner references are optional lifecycle metadata, not authorization ownership.** Generic resources may carry `metadata.ownerReferences`: UID-authoritative typed references to other resources that supplement, but never change, the resource's structural parent or URL. Admission requires each reference's kind, canonical name, and UID to resolve to the same live resource; garbage collection follows the UID, so deleting and recreating the same name never transfers lifecycle ownership. Deleting an owner initiates finalizer-respecting deletion of its dependents. `blockOwnerDeletion` defaults to `false`; when explicitly true, that dependent retains the owner's aggregate cascade finalizer until it is collected. Owner-reference graphs must be acyclic. Owner references confer no permissions, do not participate in subject expansion, label inheritance, or policy evaluation, and are never inferred or stamped by the generic API.

A `GroupMembership` may omit owner references, in which case deleting its named User leaves the marker in place and recreating that User name deliberately reactivates the name-bound membership. Alternatively, it may carry exactly one owner reference, which must identify that same User; deleting that User then garbage-collects the membership. Backend-managed JIT or directory synchronization should attach this reference when it owns the membership lifecycle, avoiding zombie membership markers. An operator creating durable policy by hand may omit it intentionally. In both cases the membership's structural parent remains its Group, so deleting the Group also deletes the membership.

**User names are stable generated identifiers, not email addresses.** A User gets an immutable, collision-resistant DNS-safe resource name such as `u-<lowercase-ulid>` (with the store's ordinary uniqueness constraint as the final authority). The generic resource-name grammar remains unchanged and does not admit `@`; relaxing it for one identity kind would either weaken every resource path or require kind-specific metadata-name parsing in the core. `User.spec` may carry presentation fields such as `displayName` and `primaryEmail`, but email is mutable, case-sensitive in troublesome ways, and non-unique across issuers, so it is never a subject key and Rise never auto-links accounts by email. `UserIdentity.spec` carries the authoritative external `issuer` and `subject`; that pair is globally unique among live UserIdentity resources. Issuers must already use their canonical URL spelling: the exact `iss` string the IdP stamps, with at most one trailing slash (OIDC compares issuers as exact strings, and IdPs differ on the slash), a lower-case host, and no default port; admission rejects aliases instead of silently repairing an authentication identifier. Both kinds also carry a platform-managed `spec.active` boolean, defaulting to `true`. An inactive User cannot log in and every already-issued token for that User fails authentication. An inactive UserIdentity blocks login through that exact upstream identity, ends the sessions established through it (each User session records the identity it was minted through), and is excluded from operator-selector matching, but does not invalidate sessions established through the User's other identities; deleting the identity ends its sessions the same way. These fields are governed resource data, but shipped policy does not let Users edit their own identity records. UI/CLI surfaces resolve and display email/name while bindings, references, URLs, and audit records use the canonical User name.

Literal subjects, `roleRef`s, and Group memberships are deliberately **name-bound**: deleting and recreating an identity or Role under the same canonical name makes existing policy refer to the replacement, matching Kubernetes RBAC's name-based references. Credentials add a separate immutable binding. Every persisted User, ServiceAccount, and Controller has a store-assigned UID, and every Rise-issued token carries both the canonical `sub` and that UID in `rise_uid`. Authentication requires both values to identify the same live, enabled resource. Recreating `serviceaccount:acme/ci` may intentionally reactivate name-bound RoleBindings for the replacement, but every token issued for the old UID fails immediately. Dynamic label subjects follow the same name-bound policy semantics. Optional owner references affect lifecycle only: they remain UID-bound and can remove a membership before a same-name User replacement appears, but never change which User a surviving membership names.
...
SSO/directory synchronization creates and deletes ordinary `GroupMembership` resources, normally attaching the synchronized User as lifecycle owner so User deletion also removes backend-managed edges. A directory-wide Group is the normal way to make every synchronized User an org member. The synchronizer authenticates with a Rise-issued principal and its membership writes pass the same grant gate as manual writes; mapping an upstream directory Group to an admin-bearing Rise Group therefore requires admin-level delegation authority. ServiceAccount inclusion in `org:<name>` remains deliberate: org-wide grants must reach CI identities too.
...
**Org bindings target only their own org.** An org-parented `RoleBinding`'s grant to any subject is intersected with live membership in that binding's *own* org — a subject receives the grant only while it is a current member of that org. An org-native subject (`Group`/`ServiceAccount`) whose baked-in org differs from the binding's org is provably foreign and contributes no grant — and because both orgs are fixed once the row exists, that is decidable at write time, so admission refuses it outright (§6.7); a `User` subject receives the grant only while a live member of the binding's org through a Group tie or the direct admin bootstrap edge above (so a user in `acme` and `beta` gets an `acme`-scoped binding's grant in `acme` alone, and loses it on leaving `acme`); `system:authenticated` inside an org binding auto-clamps to that org's authenticated members and cannot expose platform-wide. A `Controller` — org-agnostic, a member of no single org — likewise receives no org-binding grant. The remaining inert combinations are *contingently* inert — a membership can change, a caller's affiliation differs per request — so they remain valid, safe policy data and are surfaced by policy auditing rather than rejected synchronously.

Because an org `RoleBinding`'s Organization is fixed by placement, its `subject` also accepts the relative form `group:<name>`, normalized against the parent before storage. This is ergonomics for the common case, not part of the boundary: the absolute form remains writable and is what the row holds. `PlatformRoleBinding`s are root-placed and normalize to `subjectMembership: Any`, allowing deliberate cross-org or platform-wide operator grants. A PlatformRoleBinding may opt back into contextual containment with `ResourceOrganization` (§4); the seeded ownership binding does so, while RuntimeClass availability and explicit non-member grants use `Any` as appropriate. This bans cross-org sharing through org bindings by construction and prevents contextual ownership from outliving org membership; a first-class tenant-authorable cross-org sharing primitive is deferred (§10).
...
There is no **authorization** ownership primitive in this model. Nothing in §1–§5 knows what an "owner" is: the evaluator sees subjects, bindings, and labels — nothing more. Generic `metadata.ownerReferences` are lifecycle-only garbage-collection edges and grant no access. Authorization "ownership" exists only as the *effect* of one binding (§6.2) that happens to grant owner-like permissions to whoever a label names. Remove that binding and the concept vanishes from the platform without touching the engine; override it (§6.5) and ownership *means something else* in that org. The label is likewise just a convenient, inspectable targeting mechanism: `rise.dev/owner` carries no authorization semantics of its own — no label key does — a key becomes access-relevant exactly when, and only for as long as, some binding's `labelSelector` references it (§6.6 step 2). What this section defines is therefore not an authorization ownership *feature* but a shipped default *convention*: a reserved key, one seeded dynamic binding, and the write-gating that any access-driving label automatically inherits. (A dedicated single-subject authorization `ownerRef` field was considered and rejected; see Alternatives considered.)
...
Semantically inert policy is an auditing concern, not a write-time validity error — with one exception, drawn at *permanent* rather than *currently* inert. Where inertness follows from the stored row alone, so that no membership change, later resource, or restored reference could ever make the binding grant something, admission rejects it: an org `RoleBinding` whose subject names a different organization is the one shape that qualifies today, because both organizations are frozen at write time. Everything §6.7 protects stays contingent and therefore admissible — a `user:` subject is live exactly while that user is affiliated, and `subjectMembership: ResourceOrganization` compares against the *resource's* organization, which varies per request. A future Role/policy auditing workflow should flag ownership labels that currently grant nobody, selectors that match no resources, recipient-boundary or `subjectMembership` combinations that are no-ops, stale references, and Allows shadowed by replacement or Deny. Explain output must show why such data contributes no effective tuple. The synchronous mutation path retains only the checks needed for safe interpretation and non-escalation: closed-schema parsing, referential integrity, structural scope/placement constraints, and the effective-delta grant gate.
...
- **A dedicated single-subject authorization `ownerRef` field alongside the Role/binding model**, inherited down the parent chain as a union. Two independent inheritance and authorization mechanisms complicate explanations, and a descendant could never fully exclude an ancestor's owner. Rejected; §6 expresses authorization ownership with Role/RoleBinding/label primitives, while an operator-authored platform Deny can still enforce a narrower hard restriction (§4, §5). This does not reject generic `metadata.ownerReferences`, whose only effect is garbage collection (§1).
...
- a reverse live membership index on `GroupMembership.metadata.name`; the generic `(parent_uid, group, kind, name)` uniqueness rule already prevents duplicate edges for the same User in one Group, and Group-to-members lookup already uses the generic `(parent_uid, group, kind)` index.

Every predicate includes the `rise.dev` API group, exact built-in Kind, and `deletion_timestamp IS NULL`, so a custom kind with the same name in another group cannot collide. Because these are expression indexes on the canonical JSONB rows, ordinary generic create/update/delete transactions maintain them automatically — no trigger-maintained mirror table or application dual-write can drift. Typed validation guarantees the indexed fields and canonical issuer form before persistence; the unique index remains the concurrency authority.

Authentication and membership use narrow `IdentityLookup`/`MembershipLookup` Postgres adapters alongside `PgStore`, backed by those indexes and returning typed facts or resource UIDs. They are not methods on the generic `ResourceStore`, are not exposed as client-selectable filters, and do not introduce identity lookup/index semantics into `rise-resource-api` or the pure authorization crate beyond the already-shared `SubjectId`. The identity indexes require storage migrations and a small Postgres adapter, but do not change URL shape, ResourceDefinition API, or authorization algebra. Generic indexed-field declarations for user-defined kinds are a separate future feature and are unnecessary for these fixed built-ins.

**Lifecycle prerequisite:** the generic resource envelope, store, and garbage collector carry optional `metadata.ownerReferences`, transactional reverse lookup, cycle prevention, finalizer-respecting dependent collection, and optional `blockOwnerDeletion` semantics. The canonical JSONB owner-reference array is the only stored representation; a GIN containment index supports reverse UID lookup without a mirror edge table or dual-write synchronization. Cycle validation covers structural parent and owner-reference edges as one lifecycle DAG, and deletion of any referenced owner starts dependent collection. Structural children and opted-in cross-tree dependents retain one aggregate cascade finalizer. An operator-only deletion-blockers subresource computes the concrete blockers from canonical rows, while best-effort structured lifecycle audit logs record why each newly tombstoned dependent entered deletion. Durable lifecycle events require a transactional outbox or Event resource in a later increment. GroupMembership admission then enforces the kind-specific optional-User-owner rule from §1. This lifecycle facility is independent of authorization evaluation and lands before GroupMembership is activated for runtime writes.

**Prerequisite refactor:** move the `ResourceStore` contract and canonical `SubjectId`, `SubjectRef`, `ResourceKind`, and `Scope` types into dep-light `rise-resource-api`, leaving `PgStore` + SQLX in `rise-resource-store-postgres`. Delegated `(create, ResourceKind, token)` and parsed authorization caps are engine concerns; workload trust validation, UID lookup, signing, `act`, and TTL live in `rise-backend-auth`. Per-item list filtering is engine work; projection is an API/server concern.
...
rise-authz (engine) ──► rise-resource-api  (envelope types + ResourceStore trait,
   defines MembershipResolver              canonical identity types, no sqlx)
        ▲                        ▲
        │                        │ impl
rise-deploy ──► rise-resource-store-postgres (PgStore + identity/membership index adapters:
...
16. **Membership expansion and lifecycle are live.** Removing a User from a
    Group removes its Group-derived access on the next request. Deleting a User
    garbage-collects GroupMemberships that name it and carry its matching owner
    reference; markers without that reference survive and deliberately apply
    again if a User with the same canonical name is recreated. Neither form
    grants access while no live User of that name exists.
