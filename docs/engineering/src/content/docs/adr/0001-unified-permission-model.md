...
**Deferred lifecycle prerequisite:** after the identity-contract increment, the generic resource envelope, store, and garbage collector gain optional `metadata.ownerReferences`, transactional reverse lookup, cycle prevention, and finalizer-respecting dependent collection. GroupMembership admission then enforces the kind-specific optional-User-owner rule from §1. This lifecycle facility is independent of authorization evaluation and lands before GroupMembership is activated for runtime writes.
