...
**Lifecycle prerequisite:** the generic resource envelope, store, and garbage collector carry optional `metadata.ownerReferences`, transactional reverse lookup, cycle prevention, and finalizer-respecting dependent collection. The canonical JSONB owner-reference array is the only stored representation; a GIN containment index supports reverse UID lookup without a mirror edge table or dual-write synchronization. Cycle validation covers structural parent and owner-reference edges as one lifecycle DAG, and deletion of any referenced owner starts dependent collection. GroupMembership admission then enforces the kind-specific optional-User-owner rule from §1. This lifecycle facility is independent of authorization evaluation and lands before GroupMembership is activated for runtime writes.
