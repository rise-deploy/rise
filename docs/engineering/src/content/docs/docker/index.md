...
When a new deployment becomes the active one for its group, how traffic moves
from the old containers to the new is governed by
`deployment_controller.cutover_strategy`:

- **`health-rolling` (default)** — the new and old containers join **one
  group-scoped Traefik load-balancer service** immediately. When the project
  sets a `health_check`, the reconciler emits Traefik health-check labels
  (`traefik.http.services.<svc>.loadbalancer.healthcheck.*`, e.g. `path`,
  `interval`, `timeout`) so Traefik routes only to servers that pass the check.
  The new deployment is **retired-old-gated** on Traefik's per-server
  ...
  health-driven rolling update, like a Kubernetes rolling update (vs. an atomic
  blue/green switch).

- **`recreate`** — only the **active** deployment is Traefik-labeled at a time.
  Rise gates routing itself: a single cutover drops the old route and stamps the
  new one (an extra container recreate). One active/routable deployment at a
  time, no overlap.
