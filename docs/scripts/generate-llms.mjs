...
const legacyPerPageContent = (page) => `# ${page.title}\n\n${page.body.replace(/^\n+/, '')}\n`;
const perPageContent = (page) => `${GENERATED_MARKER}\n${legacyPerPageContent(page)}`;
  ...
  for (const site of SITES) {
    ...
    for (const p of all) {
      ...
      if (existsSync(dest)) {
        ...
        const isLegacyGenerated = existing === legacyPerPageContent(p);
