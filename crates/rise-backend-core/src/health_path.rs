//! The opt-in HTTP health-probe policy, shared by every deployment backend.
//!
//! Resolving a container spec's `health_check` to an effective probe path is
//! backend-agnostic: it decides *whether* and *where* to probe, not how. Each
//! backend then applies the result in its own idiom (Traefik load-balancer
//! health-check labels on Docker and ECS, readiness probes on Kubernetes).
