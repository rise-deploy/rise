//! ECS task sizing: mapping Rise's free-form `cpu`/`memory` requests onto what
//! the task definition may declare.
//!
//! Rise lets users ask for any quantity (`"500m"`, `"1.5"`, `"768Mi"`, or a
//! `request-limit` range). What ECS does with that depends on the capacity.
//!
//! **Fargate** accepts only a fixed table of pairs and rejects anything else at
//! `RegisterTaskDefinition`. Rather than surface that as an opaque AWS error at
//! reconcile time, we resolve the request here — at deploy time, against a pure
//! table — and either round **up** to the smallest combination that satisfies
//! both dimensions or reject with an error naming what would work.
//!
//! **EC2** capacity has no such table: task-level CPU and memory take arbitrary
//! values, bounded by what the container instance has rather than by a fixed
//! set. There the request passes through exactly as asked, and placement — not
//! this module — is what fails if no instance can host it.
//!
...
//! runs as 0.5 vCPU / 1 GB on Fargate. Callers surface [`TaskSize::rounded_up`]
//! so that is visible rather than a silent 4× on the memory line of an invoice.
...
/// Resolve a Rise `cpu`/`memory` request into what the task definition declares.
///
...
///
/// On Fargate, returns the smallest table combination satisfying both
/// dimensions, and errors when the request exceeds the largest one, naming the
/// maximum — an actionable message beats `RegisterTaskDefinition` failing with
/// "Invalid CPU or memory". On EC2 there is no table, so the request is carried
/// through as asked and never flagged as rounded.
pub fn resolve(cpu: &str, memory: &str, compatibility: Compatibility) -> Result<TaskSize> {
    ...
    // MiB count overflows u32 fits no task on either capacity, and a truncating
    // cast would silently wrap it down to a small valid one -- under-provisioning
    // a workload that should be rejected outright. Treat overflow as "too large".
    let too_large = || {
        if !compatibility.uses_fargate_size_table() {
            return anyhow::anyhow!(
                "cpu {cpu:?} / memory {memory:?} is too large to express as an ECS \
                 task size. Reduce the request."
            );
        }
    ...
    // EC2 declares whatever was asked for. There is no table to round onto, so
    // rounding up here would over-provision against an instance's real capacity
    // and make placement fail for a request that would have fitted.
    if !compatibility.uses_fargate_size_table() {
        return Ok(TaskSize {
            cpu_units: want_cpu_units,
            memory_mib: want_mib,
            rounded_up: false,
        });
    }
...
mod tests {
    ...
    fn assert_valid_combination(size: TaskSize) {
        let row = TABLE
            .iter()
    ...
    fn rise_defaults_round_up_to_half_vcpu_and_one_gib() {
        ...
        let size = resolve("500m", "256Mi", Compatibility::Fargate).expect("defaults must resolve");
    ...
    fn exact_table_hit_is_not_flagged_as_rounded() {
        ...
        let size = resolve("1", "2Gi", Compatibility::Fargate).expect("exact combination resolves");
    ...
    fn memory_beyond_a_row_ceiling_moves_to_the_next_cpu_row() {
        ...
        let size =
            resolve("250m", "3Gi", Compatibility::Fargate).expect("resolves by climbing a row");
        assert_eq!(size.cpu_units, 512);
    ...
    fn memory_rounds_up_to_the_step_boundary_never_down() {
        ...
        let size = resolve("500m", "1500Mi", Compatibility::Fargate).expect("resolves");
        assert_eq!(size.cpu_units, 512);
        assert_eq!(size.memory_mib, 2048);
        assert_valid_combination(size);
    }
    ...
    fn the_256_row_skips_the_invalid_1536_value() {
        ...
        let size = resolve("200m", "1100Mi", Compatibility::Fargate).expect("resolves");
    ...
    fn range_form_uses_the_limit_half() {
        ...
        let size = resolve("250m-1", "512Mi-2Gi", Compatibility::Fargate).expect("range resolves");
        assert_eq!(size.cpu_units, 1024);
        assert_eq!(size.memory_mib, 2048);
        assert_valid_combination(size);
    }
    ...
    fn a_request_past_the_u32_boundary_is_rejected_not_wrapped() {
        ...
        let err = resolve("4194305", "256Mi", Compatibility::Fargate)
            .expect_err("beyond u32 of CPU units");
        assert!(
            err.to_string()
    ...
    fn oversized_request_is_rejected_with_an_actionable_message() {
        ...
        let err = resolve("32", "256Gi", Compatibility::Fargate)
            .expect_err("beyond the largest task size");
    ...
    fn every_row_boundary_resolves_to_itself() {
        ...
        for row in TABLE {
            ...
            for mib in [row.min_mib, row.max_mib] {
                let size = resolve(&cpu, &format!("{mib}Mi"), Compatibility::Fargate)
    ...
    #[test]
    fn ec2_carries_the_request_through_without_rounding() {
        // The same defaults that Fargate raises 4x to 0.5 vCPU / 1 GiB are legal
        // as asked on EC2, where the bound is the container instance rather than
        // a table. Rounding here would over-provision against real capacity and
        // fail placement for a request that would have fitted.
        let size = resolve("500m", "256Mi", Compatibility::Ec2).expect("resolves");
        assert_eq!(size.cpu_units, 512);
        assert_eq!(size.memory_mib, 256);
        assert!(!size.rounded_up);
    }

    #[test]
    fn ec2_admits_a_combination_the_fargate_table_rejects() {
        // 0.25 vCPU / 3Gi has no Fargate row: the 256 row tops out at 2048 MiB,
        // so Fargate must climb to 512 units. EC2 has no such constraint.
        let ec2 = resolve("250m", "3Gi", Compatibility::Ec2).expect("resolves");
        assert_eq!(ec2.cpu_units, 256);
        assert_eq!(ec2.memory_mib, 3072);
        assert!(!ec2.rounded_up);

        let fargate = resolve("250m", "3Gi", Compatibility::Fargate).expect("resolves");
        assert_eq!(fargate.cpu_units, 512);
        assert!(fargate.rounded_up);
    }

    #[test]
    fn ec2_uses_the_limit_half_of_a_range_exactly_as_fargate_does() {
        // A public Rise input must mean the same ceiling on every capacity.
        let size = resolve("250m-1", "512Mi-2Gi", Compatibility::Ec2).expect("resolves");
        assert_eq!(size.cpu_units, 1024);
        assert_eq!(size.memory_mib, 2048);
    }

    #[test]
    fn ec2_still_rejects_an_unrepresentable_request_rather_than_wrapping() {
        // No table to exceed, but a truncating cast would still wrap this down
        // to a small size and silently under-provision.
        let err =
            resolve("4194305", "256Mi", Compatibility::Ec2).expect_err("beyond u32 of CPU units");
        let msg = err.to_string();
        assert!(msg.contains("too large"), "{msg}");
        // The Fargate table is not the reason on this capacity, so it must not
        // be what the operator is told to reduce against.
        assert!(!msg.contains("Fargate"), "misleading message: {msg}");
    }

    #[test]
    fn a_bad_quantity_is_refused_on_either_capacity() {
        for compatibility in [Compatibility::Fargate, Compatibility::Ec2] {
            assert!(resolve("banana", "256Mi", compatibility).is_err());
            assert!(resolve("500m", "banana", compatibility).is_err());
        }
    }
}
