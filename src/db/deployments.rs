...
/// Schedule when this deployment's workload-identity token Secret next needs
/// re-minting. The Kubernetes sync webhook sets it to (mint time + 2/3 * TTL)
/// each time it mints the tokens; the identity-refresh controller resyncs the
/// project only once this is due. (Docker re-mints on its own reconcile loop and
/// never sets this.)
#[cfg(feature = "backend")]
pub async fn set_identity_refresh_due_at(
    pool: &PgPool,
    id: Uuid,
    due_at: DateTime<Utc>,
) -> Result<()> {
    sqlx::query!(
        "UPDATE deployments SET identity_token_refresh_due_at = $2 WHERE id = $1",
        id,
        due_at
    )
    .execute(pool)
    .await
    .context("Failed to set identity refresh due time")?;
...
#[cfg(feature = "backend")]
pub async fn list_due_identity_refresh(pool: &PgPool) -> Result<Vec<(Uuid, String)>> {
    let rows = sqlx::query!(
        r#"
    ...
    .fetch_all(pool)
    .await
    .context("Failed to list deployments due for identity refresh")?;
