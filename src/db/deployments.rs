...
pub struct CreateDeploymentParams<'a> {
    ...
    /// Registry image reference allocated at deploy creation.
    /// See [`crate::db::models::Deployment::image_path`].
...
pub async fn list_for_project(pool: &PgPool, project_id: Uuid) -> Result<Vec<Deployment>> {
    let deployments = sqlx::query_as!(
        ...
        SELECT
            ...
            image, image_digest, image_path, rolled_back_from_deployment_id,
...
pub async fn create(pool: &PgPool, params: CreateDeploymentParams<'_>) -> Result<Deployment> {
    ...
    let deployment = sqlx::query_as!(
        Deployment,
        r#"
        INSERT INTO deployments (deployment_id, project_id, created_by_id, status, image, image_digest, image_path, rolled_back_from_deployment_id, deployment_group, environment_id, expires_at, http_port, is_active, job_url, pull_request_url, replicas, cpu, memory)
