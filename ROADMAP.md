...
- **Phase 1 — Add the exchange endpoint** (effectively complete, additive).
  ...
  covers 1A, 1B, and 1C. The Phase-1 remainder is the deprecation metric (1D):
  a per-`(issuer, sub)` counter surfaced as a `rise::deprecation` `tracing`
  event, plus committing the `auth.allow_raw_external_tokens` flip to 0.25.0.
...
- **Phase 3 cannot land until the deprecation metric in Phase 1 shows raw-
  ...
  metric (counted per `(issuer, sub)`) tells operators when it's safe to flip.
...
- [ ] **PR 1D — Deprecation metric + operator docs.** The structured
  ...
  turns it into a real deprecation *metric*: a per-`(issuer, sub)` counter
  (`src/server/auth/deprecation.rs`) surfaced as a metric-shaped `tracing`
  event (`target=rise::deprecation`, `metric=raw_external_token`) plus a
  periodic aggregate rollup (`metric=raw_external_token_total`) — Rise has no
  metrics endpoint, so logs are the metric transport. It also commits the flip:
  `auth.allow_raw_external_tokens` defaults to `false` starting in **0.25.0**
  (settings doc comment, `authentication.md`, and `upgrade-notes.md` updated;
  the [#374](https://github.com/rise-deploy/rise/issues/374) drain gate names
