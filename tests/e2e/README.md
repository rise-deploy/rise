...
4. **A public hosted zone for `dns_zone_name`, with the domain delegated to
   it.** `bootstrap/` reads the zone; it does not create one. Registering the
   domain through Route 53 Domains gives you both halves at once — the zone,
   and the delegation to it. Anything else means creating the zone yourself and
   setting its nameservers at the registrar. Never leave two public zones for
   the same domain: both are valid, they have different nameservers, and
